...
[CI](../../../../.github/workflows/ci.yml) 中必需的 Linux 和 Windows 任务在故障转移变量未设置时使用 GitHub 托管 Runner。托管 Runner 上的消费端任务与覆盖率任务采用比专用 Runner 更小的工作线程及门禁并发预算，使大量启动进程的测试能在既定期限内完成。Windows tools-catalog 编译测试使用覆盖率任务配置的测试时间预算，不再以 30 秒覆盖它。Python 宽结构抗恶意测试提高了执行时间上限，以兼顾低并发托管 Runner 在覆盖率插桩下的运行耗时。JSON 存储后端对 Windows 上的瞬态文件替换错误（`EACCES`、`EBUSY`、`EPERM`）采用有界退避重试，会话投影缓存跨版本测试显式等待手动写入刷盘，避免并发文件锁丢失重写后的记录。npm 解析基准在删除目录时采用有界退避重试，避免 Windows Runner 异步释放文件句柄导致 `EPERM` 失败。无头 DeepSeek 默认值测试在结束响应流前等待标题请求到达，并将流空闲超时提升至 1 秒，以避免托管 Runner 上的事件循环延迟触发多余重试。变量值 `enterprise` 选择原有专用标签；`selfhosted` 和 `blacksmith` 保留各自的显式路由。Cloudflare 预览仅在 `deepseek-ai/deepseek-harness` 运行；可信的发布预演也允许该仓库和 `deepseek-harness/deepseek-harness` 使用自托管池。自动合并上游的工作流已移除，因此同步 Fork 需要显式 Git 操作。
